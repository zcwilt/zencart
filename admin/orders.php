...
//    $orders_query_numrows = '';
                ...
                <td class="dataTableContent"><?php echo '<a href="' . zen_href_link(FILENAME_CUSTOMERS, 'cID=' . $orders->fields['customers_id'], 'NONSSL') . '">' . zen_image(DIR_WS_ICONS . 'preview.gif', ICON_PREVIEW . ' ' . TABLE_HEADING_CUSTOMERS) . '</a>&nbsp;' . htmlspecialchars($orders->fields['customers_name']) . ($orders->fields['customers_company'] != '' ? '<br>' . htmlspecialchars($orders->fields['customers_company'], ENT_COMPAT, CHARSET, TRUE) : ''); ?></td>
...
<?php } ?>
                ...
                <td class="dataTableContent text-center"<?php if (!empty($order_comments)) echo ' title="' . htmlspecialchars($order_comments, ENT_COMPAT, CHARSET, TRUE) . '"'; ?>><?php if (!empty($order_comments)) echo zen_image(DIR_WS_IMAGES . 'icon_yellow_on.gif', '', 16, 16); ?></td>
<?php
  // -----
  // A watching observer can provide an associative array in the form:
  //
