...
class ZenAiAssistDoctorService
{
    ...
    public function diagnose(string $path): array
    {
        ...
        return [
            ...
            'runtime_context' => $installedState['runtime_context'] ?? null,
            'checks' => [
                'manifest' => $manifest,
                'installer' => $installer,
    ...
    private function matchInstalledPlugin(string $pluginKey, string $pluginVersion): array
    {
        ...
        $runtimeContext = is_array($plugins['runtime_context'] ?? null)
            ? $plugins['runtime_context']
            : $this->runtimeInspector->buildRuntimeContext(
                $runtimeState,
                is_array($plugins['warnings'] ?? null) ? $plugins['warnings'] : [],
                (bool)($plugins['inspection_available'] ?? false),
                (string)($plugins['runtime_state_detail'] ?? '')
            );
        ...
        if ($runtimeState !== 'available') {
            return [
                ...
                'runtime_state_category' => (string)($runtimeContext['category'] ?? 'degraded'),
                'runtime_state_detail' => (string)($runtimeContext['detail'] ?? ''),
                'inspection_available' => (bool)($runtimeContext['inspection_available'] ?? false),
                'warnings' => $warnings,
        ...
        foreach ($pluginRows as $plugin) {
            ...
            return [
                ...
                'plugin' => $plugin,
                'runtime_state' => $runtimeState,
                'runtime_state_category' => (string)($runtimeContext['category'] ?? 'available'),
                'runtime_state_detail' => (string)($runtimeContext['detail'] ?? ''),
                'inspection_available' => (bool)($runtimeContext['inspection_available'] ?? true),
                'warnings' => $warnings,
    ...
    private function classifyFindingSeverity(string $finding): string
    {
        $patterns = [
            ...
            'warning' => [
                ...
                '/language file does not define any language keys\./i',
                ...
                '/has no matching installer `zen_register_admin_page\(\)` registration\./i',
                '/installer registration references language key /i',
                '/installer registration references `FILENAME_/i',
                '/does not follow the expected encapsulated plugin observer naming patterns\./i',
                '/does not declare a class extending `base`\./i',
                '/does not appear to attach to notifications or expose update handlers\./i',
                '/loader file `[^`]+` does not define any recognized loader entries\./i',
                '/loader file `[^`]+` references missing file /i',
                '/loader file `[^`]+` defines `classInstantiate` without a `className`\./i',
                '/loader file `[^`]+` does not guard direct access with `IS_ADMIN_FLAG`\./i',
            ],
            'info' => [
