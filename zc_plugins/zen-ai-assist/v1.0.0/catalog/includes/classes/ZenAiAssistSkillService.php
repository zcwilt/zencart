...
class ZenAiAssistSkillService
{
    ...
    public function __construct(string|ZenAiAssistContentRegistry $skillsDirectory)
    {
        if ($skillsDirectory instanceof ZenAiAssistContentRegistry) {
            $this->registry = $skillsDirectory;
            return;
        }
    ...
    private function loadSkills(): array
    {
        ...
        foreach ($this->skillSources() as $source) {
            $catalogSkills = $this->loadCatalogSkills($source);
            foreach ($catalogSkills as $skill) {
                $skillId = (string)($skill['id'] ?? '');
                if ($skillId === '' || isset($knownSkillIds[$skillId])) {
                    continue;
                }
            ...
            foreach ($this->loadFallbackMarkdownSkills($source, array_keys($knownSkillIds)) as $skill) {
                $skillId = (string)($skill['id'] ?? '');
                if ($skillId === '' || isset($knownSkillIds[$skillId])) {
                    continue;
                }
    ...
    private function loadCatalogSkills(array $source): array
    {
        $skillsDirectory = rtrim((string)($source['skills_dir'] ?? ''), '/\\') . '/';
        if ($skillsDirectory === '/') {
            return [];
        }

        $path = $skillsDirectory . 'catalog.json';
        if (!is_file($path)) {
            return [];
        }
        ...
        foreach ($skills as $skill) {
            ...
            $contentPath = $contentFile === '' ? null : $skillsDirectory . ltrim($contentFile, '/\\');
            $content = '';
    ...
    private function loadFallbackMarkdownSkills(array $source, array $catalogSkillIds): array
    {
        $skillsDirectory = rtrim((string)($source['skills_dir'] ?? ''), '/\\') . '/';
        if ($skillsDirectory === '/' || !is_dir($skillsDirectory)) {
            return [];
        }
        ...
        foreach (glob($skillsDirectory . '*.md') ?: [] as $path) {
    ...
    private function summarizeSkill(array $skill): array
    {
        return [
            ...
            'tags' => $this->normalizeList($skill['tags'] ?? []),
            'path' => $skill['path'] ?? null,
            'source' => is_array($skill['source'] ?? null) ? $skill['source'] : null,
        ];
    }

    private function skillSources(): array
    {
        if ($this->registry !== null) {
            return $this->registry->skillSources();
        }
        ...
        return [[
            ...
            'skills_dir' => $this->skillsDirectory,
        ]];
    }
