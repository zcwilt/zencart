...
//
  ...
  if (zen_not_null($action)) {
    switch ($action) 
	{
	  case 'set_redeem_ratio':
		UpdateRedeemRatioRecord(SCOPE_GLOBAL,0,zen_db_prepare_input((float)$_POST['global_redeem_ratio']));
		break;
      case 'insert':
      case 'save':
        ...
        $group_redeem_ratio = zen_db_prepare_input((float)$_POST['group_redeem_ratio']);
        if ($group_name) {
		...
		if($group_redeem_ratio)
		 UpdateRedeemRatioRecord(SCOPE_GROUP,(int)$group_id,$group_redeem_ratio);
        zen_redirect(zen_href_link(FILENAME_ADMIN_GROUP_REWARD_POINTS_REDEEM, (isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . 'gID=' . $group_id));
		break;
      case 'deleteconfirm':
        if (zen_admin_demo()) {
          $_GET['action']= '';
          $messageStack->add_session(ERROR_ADMIN_DEMO, 'caution');
          zen_redirect(zen_href_link(FILENAME_ADMIN_GROUP_REWARD_POINTS_REDEEM, 'page=' . $_GET['page']));
        }
		...
		DeleteRewardPointRecord(SCOPE_GROUP,(int)$group_id);
        zen_redirect(zen_href_link(FILENAME_ADMIN_GROUP_REWARD_POINTS_REDEEM, 'page=' . $_GET['page']));
		break;
    }
  }
...
<?php
	...
	echo zen_draw_form('set_global_redeem_points',FILENAME_ADMIN_GROUP_REWARD_POINTS_REDEEM,'action=set_redeem_ratio','post').zen_hide_session_id();
	echo TEXT_SET_GLOBAL_POINTS_RATIO.'&nbsp;'.zen_draw_input_field('global_redeem_ratio',$global_reward_point_record->fields[FIELD_REDEEM_RATIO],zen_set_field_length(TABLE_REWARD_MASTER,FIELD_REDEEM_RATIO));
	echo '</form>';
?>
	 </td>
    </tr>
...
<?php
  ...
  while (!$groups->EOF) {
	...
	$reward_points=GetRewardPointRecord(SCOPE_GROUP,$groups->fields['group_id']);
	
	if (isset($gInfo) && is_object($gInfo) && ($groups->fields['group_id'] == $gInfo->group_id)) {
      echo '              <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . zen_href_link(FILENAME_ADMIN_GROUP_REWARD_POINTS_REDEEM, 'page=' . $_GET['page'] . '&gID=' . $groups->fields['group_id'] . '&action=edit') . '\'">' . "\n";
    } else {
      echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . zen_href_link(FILENAME_ADMIN_GROUP_REWARD_POINTS_REDEEM, 'page=' . $_GET['page'] . '&gID=' . $groups->fields['group_id'] . '&action=edit') . '\'">' . "\n";
    }
?>
                ...
                <td class="dataTableContent"><?php echo $reward_points->fields['redeem_ratio']; ?></td>
                <td class="dataTableContent" align="right">
                  <?php echo '<a href="' . zen_href_link(FILENAME_ADMIN_GROUP_REWARD_POINTS_REDEEM, 'page=' . $_GET['page'] . '&gID=' . $groups->fields['group_id'] . '&action=edit') . '">' . zen_image(DIR_WS_IMAGES . 'icon_edit.gif', ICON_EDIT) . '</a>'; ?>
                  <?php echo '<a href="' . zen_href_link(FILENAME_ADMIN_GROUP_REWARD_POINTS_REDEEM, 'page=' . $_GET['page'] . '&gID=' . $groups->fields['group_id'] . '&action=delete') . '">' . zen_image(DIR_WS_IMAGES . 'icon_delete.gif', ICON_DELETE) . '</a>'; ?>
                  <?php if (isset($gInfo) && is_object($gInfo) && ($groups->fields['group_id'] == $gInfo->group_id)) { echo zen_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', ''); } else { echo '<a href="' . zen_href_link(FILENAME_ADMIN_GROUP_REWARD_POINTS_REDEEM, zen_get_all_get_params(array('gID')) . 'gID=' . $groups->fields['group_id']) . '">' . zen_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>
                </td>
              </tr>
<?php
    $groups->MoveNext();
...
?>
              <tr>
                <td align="right" colspan="4" class="smallText"><?php echo '<a href="' . zen_href_link(FILENAME_ADMIN_GROUP_REWARD_POINTS_REDEEM, 'page=' . $_GET['page'] . '&gID=' . $gInfo->group_id . '&action=new') . '">' . zen_image_button('button_insert.gif', IMAGE_INSERT) . '</a>'; ?></td>
              </tr>
<?php
  }
?>
...
<?php
  ...
  switch ($action) {
    case 'new':
      ...
      $contents = array('form' => zen_draw_form('group_pricing', FILENAME_ADMIN_GROUP_REWARD_POINTS_REDEEM, 'action=insert', 'post'));
      ...
      $contents[] = array('text' => '<br>' . TEXT_GROUP_REDEEM_RATIO . '<br>' . zen_draw_input_field('group_redeem_ratio', ''));
      $contents[] = array('align' => 'center', 'text' => '<br>' . zen_image_submit('button_save.gif', IMAGE_SAVE) . ' <a href="' . zen_href_link(FILENAME_ADMIN_GROUP_REWARD_POINTS_REDEEM, 'page=' . $_GET['page'] . '&gID=' . $_GET['gID']) . '">' . zen_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
      break;
    case 'edit':
      $heading[] = array('text' => '<b>' . TEXT_HEADING_EDIT_PRICING_GROUP . '</b>');

      $contents = array('form' => zen_draw_form('group_pricing', FILENAME_ADMIN_GROUP_REWARD_POINTS_REDEEM, 'page=' . $_GET['page'] . '&gID=' . $gInfo->group_id . '&action=save', 'post'));
      ...
      $contents[] = array('text' => '<br>' . TEXT_GROUP_REDEEM_RATIO . '<br>' . zen_draw_input_field('group_redeem_ratio', $reward_points->fields['redeem_ratio']));
      $contents[] = array('align' => 'center', 'text' => '<br>' . zen_image_submit('button_save.gif', IMAGE_SAVE) . ' <a href="' . zen_href_link(FILENAME_ADMIN_GROUP_REWARD_POINTS_REDEEM, 'page=' . $_GET['page'] . '&gID=' . $gInfo->group_id) . '">' . zen_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
      break;
    case 'delete':
      $heading[] = array('text' => '<b>' . TEXT_HEADING_DELETE_PRICING_GROUP . '</b>');

      $contents = array('form' => zen_draw_form('group_pricing', FILENAME_ADMIN_GROUP_REWARD_POINTS_REDEEM, 'page=' . $_GET['page'] . '&gID=' . $gInfo->group_id . '&action=deleteconfirm'));
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . zen_image_submit('button_delete.gif', IMAGE_DELETE) . ' <a href="' . zen_href_link(FILENAME_ADMIN_GROUP_REWARD_POINTS_REDEEM, 'page=' . $_GET['page'] . '&gID=' . $gInfo->group_id) . '">' . zen_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
      break;
    default:
      if (isset($gInfo) && is_object($gInfo)) {
        $heading[] = array('text' => '<b>' . $gInfo->group_name . '</b>');

        $contents[] = array('align' => 'center', 'text' => '<a href="' . zen_href_link(FILENAME_ADMIN_GROUP_REWARD_POINTS_REDEEM, 'page=' . $_GET['page'] . '&gID=' . $gInfo->group_id . '&action=edit') . '">' . zen_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a href="' . zen_href_link(FILENAME_ADMIN_GROUP_REWARD_POINTS_REDEEM, 'page=' . $_GET['page'] . '&gID=' . $gInfo->group_id . '&action=delete') . '">' . zen_image_button('button_delete.gif', IMAGE_DELETE) . '</a>');
