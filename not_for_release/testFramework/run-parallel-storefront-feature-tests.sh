...
run_test_file() {
    ...
    (
        ...
        if "$PHP_BIN" "$PHPUNIT_BIN" --configuration "$ROOT_DIR/phpunit.xml" --testsuite FeatureStore --group parallel-candidate "${EXTRA_PHPUNIT_ARGS[@]}" "$file" >"$output_file" 2>&1; then
            echo 0 >"$status_file"
        else
            echo $? >"$status_file"
        fi
