...
- When registering a `configuration` (or `product_type_layout`) entry via `addConfigurationKey()`/`updateConfigurationKey()`, prefer the `'renderer'` property over the legacy `'set_function'`/`'use_function'` strings for new keys: build it with the inherited `configFieldRenderer($rendererKey, $params = [], $formatterKey = null)` helper, e.g. `'renderer' => $this->configFieldRenderer('zen_cfg_select_option', ['options' => ['true', 'false']])`. The legacy keys still work unmodified and are never removed, but `renderer` resolves through a typed `Zencart\ConfigField\ConfigFieldRendererInterface`/`ConfigFieldFormatterInterface` class instead of an `eval()`'d string. See `Zencart\ConfigField\CoreRegistryBootstrap` for the closed set of core renderer/formatter keys (all named after the `zen_cfg_*`/`zen_get_*` function they wrap), or register your own class under a new key via a `catalog|admin/includes/auto_loaders/config.*.php` bootstrap file (loadpoint 200+, since core's registry is built at breakpoint 176) that calls `$zcConfigFieldRegistry->registerRendererClass('mykey', MyRenderer::class)`/`registerFormatterClass()` — the class-string form validates the interface immediately but defers instantiation until the key is actually resolved, so your renderer isn't constructed on every admin page load. `registerRenderer()`/`registerFormatter()` (instance form) are also available if you already have a constructed object to hand.
